...
wheelCounter = 0 #Counter to prevent pitchwheel off spam
KeyCounter = [0]*16 #counter to prevent note spam
class GUI:
	def __init__(self, master, config):
		...
		fileMenu.add_command(label="Controller", command=lambda:Controller(self.outport, self.config, self.instruString, self.window))
		...
		optionsMenu.add_command(label="Axis Manager", command=self.axisManager)
		...
		# Any key that's pressed activates hit(), which plays notes and changes the counter.
		master.bind("<KeyPress>", self.hit)
		# When the key is released it activates release(), which turns off the note and changes the counter. 
		...
		if (os.path.isfile('settings.ini')):
			...
			axisString = self.config.get('Axis Binds', 'Axis')
			effectString = self.config.get('Axis Binds', 'Effects')
			self.axis = axisString.split(",")
			self.effect = effectString.split(",")
	...
	def binding(self, key, instr, note, num):
		...
		noteList = ['C 3', 'C# 3', 'D 3', 'D# 3', 'E 3', 'F 3', 'F# 3', 'G 3', 'G# 3', 'A 3', 'A# 3', 'B 3',
	...
	def axisManager(self):
		...
		# Window setup.
		self.axisMgr = Toplevel(self.window)
		self.axisMgr.grab_set()
		self.axisMgr.focus_set()
		self.axisMgr.title("axis Manager")
		self.axisMgr.resizable(0,0)
		
		# Sets up text label for binding instructions.
		instructions = Label(self.axisMgr, text="Click a button to access binding settings.")
		instructions.grid(columnspan=3)
		
		# Clear All button for removing all binds.
		clear = Button(self.axisMgr, text="Clear All", command=self.clearAxisBinds)
		clear.grid(column=3)
		...
		c = 0
		button=[None]*16
		self.axisbind = [None]*16
		
		for x in range(16):
			self.axisbind[x] = StringVar()
			self.axisbind[x].set("Axis: {0} \n Effect: {1} ".format(self.axis[x], self.effect[x]))
		
		button[0] = Button(self.axisMgr, textvariable=self.axisbind[0], command=lambda: self.axisbinding(self.axis[0], self.effect[0], 0))
		button[1] = Button(self.axisMgr, textvariable=self.axisbind[1], command=lambda: self.axisbinding(self.axis[1], self.effect[1], 1))
		button[2] = Button(self.axisMgr, textvariable=self.axisbind[2], command=lambda: self.axisbinding(self.axis[2], self.effect[2], 2))
		button[3] = Button(self.axisMgr, textvariable=self.axisbind[3], command=lambda: self.axisbinding(self.axis[3], self.effect[3], 3))
		button[4] = Button(self.axisMgr, textvariable=self.axisbind[4], command=lambda: self.axisbinding(self.axis[4], self.effect[4], 4))
		button[5] = Button(self.axisMgr, textvariable=self.axisbind[5], command=lambda: self.axisbinding(self.axis[5], self.effect[5], 5))
		button[6] = Button(self.axisMgr, textvariable=self.axisbind[6], command=lambda: self.axisbinding(self.axis[6], self.effect[6], 6))
		button[7] = Button(self.axisMgr, textvariable=self.axisbind[7], command=lambda: self.axisbinding(self.axis[7], self.effect[7], 7))
		button[8] = Button(self.axisMgr, textvariable=self.axisbind[8], command=lambda: self.axisbinding(self.axis[8], self.effect[8], 8))
		button[9] = Button(self.axisMgr, textvariable=self.axisbind[9], command=lambda: self.axisbinding(self.axis[9], self.effect[9], 9))
		button[10] = Button(self.axisMgr, textvariable=self.axisbind[10], command=lambda: self.axisbinding(self.axis[10], self.effect[10], 10))
		button[11] = Button(self.axisMgr, textvariable=self.axisbind[11], command=lambda: self.axisbinding(self.axis[11], self.effect[11], 11))
		button[12] = Button(self.axisMgr, textvariable=self.axisbind[12], command=lambda: self.axisbinding(self.axis[12], self.effect[12], 12))
		button[13] = Button(self.axisMgr, textvariable=self.axisbind[13], command=lambda: self.axisbinding(self.axis[13], self.effect[13], 13))
		button[14] = Button(self.axisMgr, textvariable=self.axisbind[14], command=lambda: self.axisbinding(self.axis[14], self.effect[14], 14))
		button[15] = Button(self.axisMgr, textvariable=self.axisbind[15], command=lambda: self.axisbinding(self.axis[15], self.effect[15], 15))
	...
	def axisbinding(self, axis, effect, num):
		self.bindWindow = Toplevel(self.axisMgr)
		self.bindWindow.title("Bind Axis Configuration")
		self.bindWindow.focus_set()
		
		# Axis binding.
		btnLabel = Label(self.bindWindow, text="Axis: ")
		btnLabel.grid(row=0, column=0)
		self.bindDisplay = StringVar()
		self.bindDisplay.set(axis)
		self.newAxis = axis
		axisBind = Button(self.bindWindow, textvariable=self.bindDisplay, command=lambda:self.move_axis(num))
		axisBind.grid(row=0, column=1)
		
		
		# Effect binding.
		effectLabel = Label(self.bindWindow, text="Effect: ")
		effectLabel.grid(row=2)
		self.effectLetter = StringVar()
		self.effectLetter.set(effect)
		effectList = ['Pitchwheel', 'Control Message', 'No Effect']
		effectBind = OptionMenu(self.bindWindow, self.effectLetter, *effectList)
		effectBind.grid(row=1, column=1)
		
		# OK and Cancel buttons.
		buttonOK = Button(self.bindWindow, text="OK", command=lambda:self.axisBindOK(num))
		buttonCancel = Button(self.bindWindow, text="Cancel", command=self.bindWindow.destroy)
		buttonOK.grid(row=3,column=1)
	...
	def axisBindOK(self, num):
		self.effect[num] = self.effectLetter.get()
		self.axisbind[num].set("Axis: {0} \n Effect: {1}".format(str(self.axis[num]), self.effect[num]))
		
		axisString = ""
		effectString = ""
		
		for x in range(16):
			axisString += (str(self.axis[x]) + ",") 
			effectString += (self.effect[x] + ",") 
			self.config['Joystick Binds'] = {'Axis':axisString, 'Effects':effectString}
		with open('settings.ini', 'w') as configfile:
			self.config.write(configfile)
	...
	def clearAxisBinds(self):
		self.axis = [0]*16
		self.effectString = ["No Effect"]*16
		...
		for x in range(16):
			AxisString += (str(self.axis[x]) + ",")
			EffectString += (self.effectString[x] + ",") 
			
		self.config['Axis Binds'] = {'Axis':AxisString, 'Effects':EffectString}
		with open('settings.ini', 'w') as configfile:
			self.config.write(configfile)
		...
		try:
			...
			for x in range(16):
				if x!=9:
					self.axisbind[x].set("Axis: {0} \n Effect: {1}".format(self.axis[x], self.effectString[x]))				
...
#--------------------------------------------------------------------------------------------------------------------------------
	...
	def hit(self, event):
		...
		if (key_on in self.key):
			...
			if (self.instruString[num][1] == ' '):
				instruNum = int(self.instruString[num][0])
			elif (self.instruString[num][2] == ' '):
				instruNum = int(self.instruString[num][:2])
			else:
				instruNum = int(self.instruString[num][:3])
			self.outport.send(mido.Message('program_change', program=instruNum-1))
			if (KeyCounter[num] == 0): #Prevnts note spam
				message = mido.Message('note_on', note=self.noteToValue(self.note[num])+self.octave, velocity=100)
				self.outport.send(message)
				KeyCounter[num] = 1
				print(message.type)
		
	#Makes the keys bahave more like a piano
	def release(self, event):
		key_off = event.char

		if (key_off in self.key):		
			num = self.key.index(key_off)
			print("woo")
			message = mido.Message('note_off', note=self.noteToValue(self.note[num])+self.octave, velocity=100)
			self.outport.send(message)		
			KeyCounter[num] = 0

class Controller:
	def __init__(self, outport, config, kbInstr, origWindow):
		...
		self.screen = Toplevel(origWindow)
		...
		if (os.path.isfile('settings.ini')):
			...
			axisString = self.config.get('Joystick Binds', 'axis')
			self.button = buttonString.split(",")
		...
		# DRAWING STEP
			...
			joystickCountDisplay = Label(self.screen, text="Number of joysticks: {0}".format(joystick_count))
			joystickCountDisplay.grid()
    
			# For each joystick:
			joystickDisplayVar = StringVar()
			joystickDisplayText = ""
			joystickDisplay = Label(self.screen, textvariable=joystickDisplayVar)
			for i in range(joystick_count):
				self.joystick = pygame.joystick.Joystick(i)
				self.joystick.init()
    
				joystickDisplayText += ("Joystick {0} \n".format(i))
    
				# Get the name from the OS for the controller/joystick.
				name = self.joystick.get_name()
				joystickDisplayText += ("Joystick name: {0} \n".format(name))
        
				# Usually axis run in pairs, up/down for one, and left/right for the other.
				joystickDisplayText += ("Number of axes: {0} \n".format(self.joystick.get_numaxes()))
           
				buttons = self.joystick.get_numbuttons()
				joystickDisplayText += ("Number of buttons: {0} \n".format(buttons))

				# Hat switch. All or nothing for direction, not like joysticks.
				# Value comes back in an array.
				hats = self.joystick.get_numhats()
				joystickDisplayText += ("Number of hats: {0} \n".format(hats))

				for i in range(hats):
					hat = self.joystick.get_hat(i)
					joystickDisplayText += ("Hat {0} value: {1} \n \n".format(i, str(hat)))
					
			# Go ahead and update the screen with what we've drawn.
			joystickDisplayVar.set(joystickDisplayText)
			joystickDisplay.grid(row=2)
	...
	def signalOnPress(self, buttonOn, i):
		if buttonOn==1 and i in self.button and self.button_was_pressed[i]==0 and self.safeCheck1==1:
			...
			instrNum=0
			if (self.instrument[num][1] == ' '):
				instrNum = int(self.instrument[num][0])
			elif (self.instrument[num][2] == ' '):
				instrNum = int(self.instrument[num][:2])
			else:
				instrNum = int(self.instrument[num][:3])
			self.outport.send(mido.Message('program_change', program=instrNum-1))
			self.outport.send(mido.Message('note_on', note=self.note[num], velocity=64))
			self.button_was_pressed[i] = 1
		elif buttonOn==0 and i in self.button and self.button_was_pressed[i]==1 and self.safeCheck2==1:
			num = self.button.index(i)
			self.outport.send(mido.Message('note_off', note=self.note[num], velocity=64))
	...
	def signalOnAxis(self, axisOn, i):

		axisEffect = self.axis.index(i)
		if axisEffect == 'Pitchwheel':
			currentEffect = 'pitchwheel'
		elif axisEffect == 'Control Message':
			currentEffect = 'control'
		elif axisEffect == 'No Effect':
			return NULL

	
		if axisOn < 0:
			self.absolute_axis = axisOn*-1
		else:
			self.absolute_axis = axisOn

		if (self.absolute_axis > .175 and axisEffect == 'Pitchwheel'):
			self.pith = -8000*axis
			self.outport.send(mido.Message('pitchwheel', pitch = int(self.pith)))
			wheelCounter = 1

		elif(self.absolute_axis > .175 and axisEffect == 'Control Message'):
			self.contr = .175*127
			self.outport.send(mido.Message('control_change', control = int(self.contr)))

		#This resets the pitch when the axis goes to nutral position 	
		elif (wheelCounter == 1):
			for x in range(16):
				self.outport.send(mido.Message('pitchwheel', pitch = 0))
			wheelCounter = 0	#So ut doesn't spam pitchwheel = 0 every frame	
	...
	def main(self):
		...
		while not done:
			...
			for i in range(buttons):
				self.signalOnPress(self.joystick.get_button(i), i)
			
			#Axis Effects	
			for i in range(axis):
				self.signalOnAxis(self.joystick.get_numaxes(i), i)	
