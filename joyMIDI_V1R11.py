...
if (os.path.isfile('settings.ini')):
    ...
    button_string = config.get('Joy Binds','inputs')
    instrument_string = config.get('Joy Binds', 'instruments')
    toneString = config.get('Joy Binds', 'notes')
    userPort = config.get('MIDI port', 'port')
    
    inputs = button_string.split(",")
    instrument = instrument_string.split(",")
...
while done==False:
    ...
    for i in range(joystick_count):
        ...
        for i in range( buttons ):
            ...
            t = 0
            while inputs[t] != " Empty" and inputs[t] != "Empty" and t < len(inputs):
                signalOnPress(notes[i], inputs[i])
                t = t + 1                                                                     
