...
class GUI:
	def __init__(self, master, config):
		...
		fileMenu.add_command(label="Controller", command=lambda:Controller(self.outport, config))
	...
	def controllerManager(self):
		...
		# Clear All button for removing all binds.
		clear = Button(self.ctrlMgr, text="Clear All", command=self.clearBinds)
		clear.grid(column=3)
		...
		for x in range(16):
			...
			self.ctrlbind[x].set("Key: {0} \n Instrument: {1} \n note: {2}".format(self.button[x], self.instruString[x], self.note[x]))
		self.ctrlbind[9].set("PERCUSSION")
			
		# This is gross but bear with me
		button[0] = Button(self.ctrlMgr, textvariable=self.ctrlbind[0], command=lambda: self.ctrlbinding(self.button[0], self.instruString[0], self.note[0], 0))
		button[1] = Button(self.ctrlMgr, textvariable=self.ctrlbind[1], command=lambda: self.ctrlbinding(self.button[1], self.instruString[1], self.note[1], 1))
		button[2] = Button(self.ctrlMgr, textvariable=self.ctrlbind[2], command=lambda: self.ctrlbinding(self.button[2], self.instruString[2], self.note[2], 2))
		button[3] = Button(self.ctrlMgr, textvariable=self.ctrlbind[3], command=lambda: self.ctrlbinding(self.button[3], self.instruString[3], self.note[3], 3))
		button[4] = Button(self.ctrlMgr, textvariable=self.ctrlbind[4], command=lambda: self.ctrlbinding(self.button[4], self.instruString[4], self.note[4], 4))
		button[5] = Button(self.ctrlMgr, textvariable=self.ctrlbind[5], command=lambda: self.ctrlbinding(self.button[5], self.instruString[5], self.note[5], 5))
		button[6] = Button(self.ctrlMgr, textvariable=self.ctrlbind[6], command=lambda: self.ctrlbinding(self.button[6], self.instruString[6], self.note[6], 6))
		button[7] = Button(self.ctrlMgr, textvariable=self.ctrlbind[7], command=lambda: self.ctrlbinding(self.button[7], self.instruString[7], self.note[7], 7))
		button[8] = Button(self.ctrlMgr, textvariable=self.ctrlbind[8], command=lambda: self.ctrlbinding(self.button[8], self.instruString[8], self.note[8], 8))
		button[9] = Button(self.ctrlMgr, textvariable=self.ctrlbind[9], command=lambda: self.ctrlbinding(self.button[9], self.instruString[9], self.note[9], 9))
		button[9].config(state="disabled", height=3)
		button[10] = Button(self.ctrlMgr, textvariable=self.ctrlbind[10], command=lambda: self.ctrlbinding(self.button[10], self.instruString[10], self.note[10], 10))
		button[11] = Button(self.ctrlMgr, textvariable=self.ctrlbind[11], command=lambda: self.ctrlbinding(self.button[11], self.instruString[11], self.note[11], 11))
		button[12] = Button(self.ctrlMgr, textvariable=self.ctrlbind[12], command=lambda: self.ctrlbinding(self.button[12], self.instruString[12], self.note[12], 12))
		button[13] = Button(self.ctrlMgr, textvariable=self.ctrlbind[13], command=lambda: self.ctrlbinding(self.button[13], self.instruString[13], self.note[13], 13))
		button[14] = Button(self.ctrlMgr, textvariable=self.ctrlbind[14], command=lambda: self.ctrlbinding(self.button[14], self.instruString[14], self.note[14], 14))
		button[15] = Button(self.ctrlMgr, textvariable=self.ctrlbind[15], command=lambda: self.ctrlbinding(self.button[15], self.instruString[15], self.note[15], 15))
	...
	def ctrlBindOK(self, num):
		self.btnInstruString[num] = self.instrument.get()
		self.btnNote[num] = self.noteLetter.get()
		self.ctrlbind[num].set("Key: {0} \n Instrument: {1} \n Note: {2}".format(str(self.button[num]), self.instrument.get(), self.noteLetter.get()))
	...
	def clearBinds(self):
		...
		try:
			...
			self.bind[0].set("Key: {0} \n Instrument: {1} \n note: {2}".format(self.key[0], self.instruString[0], self.note[0]))
			self.bind[1].set("Key: {0} \n Instrument: {1} \n note: {2}".format(self.key[1], self.instruString[1], self.note[1]))
			self.bind[2].set("Key: {0} \n Instrument: {1} \n note: {2}".format(self.key[2], self.instruString[2], self.note[2]))
			self.bind[3].set("Key: {0} \n Instrument: {1} \n note: {2}".format(self.key[3], self.instruString[3], self.note[3]))
			self.bind[4].set("Key: {0} \n Instrument: {1} \n note: {2}".format(self.key[4], self.instruString[4], self.note[4]))
			self.bind[5].set("Key: {0} \n Instrument: {1} \n note: {2}".format(self.key[5], self.instruString[5], self.note[5]))
			self.bind[6].set("Key: {0} \n Instrument: {1} \n note: {2}".format(self.key[6], self.instruString[6], self.note[6]))
			self.bind[7].set("Key: {0} \n Instrument: {1} \n note: {2}".format(self.key[7], self.instruString[7], self.note[7]))
			self.bind[8].set("Key: {0} \n Instrument: {1} \n note: {2}".format(self.key[8], self.instruString[8], self.note[8]))
			#self.bind[9].set("Key: {0} \n Instrument: {1} \n note: {2}".format(self.key[9], self.instruString[9], self.note[9]))
			self.bind[10].set("Key: {0} \n Instrument: {1} \n note: {2}".format(self.key[10], self.instruString[10], self.note[10]))
			self.bind[11].set("Key: {0} \n Instrument: {1} \n note: {2}".format(self.key[11], self.instruString[11], self.note[11]))
			self.bind[12].set("Key: {0} \n Instrument: {1} \n note: {2}".format(self.key[12], self.instruString[12], self.note[12]))
			self.bind[13].set("Key: {0} \n Instrument: {1} \n note: {2}".format(self.key[13], self.instruString[13], self.note[13]))
			self.bind[14].set("Key: {0} \n Instrument: {1} \n note: {2}".format(self.key[14], self.instruString[14], self.note[14]))
			self.bind[15].set("Key: {0} \n Instrument: {1} \n note: {2}".format(self.key[15], self.instruString[15], self.note[15]))	
...
class Controller:
	def __init__(self, outport, config):
		...
		# It is equal to 1 when it has gone one frame without being pressed, and 2 when it is pressed.
	...
	def main(self):
		while not self.done:
			...
			TextPrint.printTo(self, self.screen, "Number of joysticks: {}".format(joystick_count) )
			...
			for i in range(joystick_count):
				...
				TextPrint.printTo(self, self.screen, "Joystick {}".format(i))
				TextPrint.indent(self)
