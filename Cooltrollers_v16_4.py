...
class Controller:
	...
	def signalOnPress(self, button_was_pressed, buttonOn, i):
		if buttonOn==1 and i in self.button and button_was_pressed==0:
			...
			self.outport.send(mido.Message('note_on', note=self.note[num], velocity=100))
			button_was_pressed = 1
			return button_was_pressed

		elif buttonOn==0 and i in self.button and button_was_pressed==1:
			num = self.button.index(i)
			self.outport.send(mido.Message('note_off', note=self.note[num], velocity=100))
			button_was_pressed = 2
			return button_was_pressed

		elif buttonOn==0 and i in self.button and button_was_pressed==2:
		...
		while self.done == False:
			...
			for i in range(joystick_count):
				...
				for i in range(axes):
					...
					if i == 0 and (self.absolute_axis*8000 > 300):
						self.pith = -8000*axis
						for x in range(16):
							self.outport.send(mido.Message('pitchwheel', pitch = int(self.pith)))
						self.axis_counter = 1	
					elif self.axis_counter == 1:
				...
				for i in range(buttons):
					button_was_pressed[i] = self.signalOnPress(button_was_pressed[i], joystick.get_button(i), i)
